 ...
 * Uses Bun runtime with @neondatabase/serverless approach for direct SQL execution
 */
...
async function main() {
  try {
    console.log('===== UPDATING SCHEMA WITH NEON SERVERLESS (BUN RUNTIME) =====');
    ...
    if (connectionTest[0]?.connected === 1) {
      console.log('u2713 Database connection successful!');
    } else {
    ...
    console.log('\nu2713 Schema update completed successfully!');
  } catch (error) {
